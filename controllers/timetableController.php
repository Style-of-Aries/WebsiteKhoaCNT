...
class timetableController
{
    ...
    public function editTkb()
    {
        ...
        // // ===== LẤY DATA =====
        // // $timetable = $this->timetableModel->getByClass_sessionId($id_buoiHoc);
        ...
        $course_classes = $this->course_classesModel->getById($id);
        $timetable = $this->timetableModel->getByCourseClassId($id);
        $class_sessions = $this->timetableModel->getByClass_sessionId($id_buoihoc);
    ...
    //     $semester = $this->semesterModel->getAll();
    //     $course_classes = $this->course_classesModel->getById($id);
    //     $class_sessions = $this->timetableModel->getByClass_sessionId($id);
    //     require_once './../views/admin/timetable/edit.php';
    // }
    public function tkb()
    {
    ...
    public function add_tkb()
    {
        ...
        if (isset($_POST['btn_add'])) {
            ...
            $course_classes = $this->timetableModel->getLecturerByCourseClass($course_class_id);

            $lecturer_id = $course_classes['lecturer_id'];
            ...
            if (empty($errors)) {
                ...
                foreach ($days as $i => $day) {
                    ...
                    for ($i = 0; $i < count($days); $i++) {
                        ...
                        $check = $this->timetableModel->checkTrungLichGvTheoTuan(
                            $lecturer_id,
                            $days[$i],
                            $sessions[$i],
                            $_POST['start_week'],
                        ...
                        if ($check) {
                            $errors['schedule'] = "Giảng viên đã có lịch vào Thứ {$days[$i]} - {$sessions[$i]} trong khoảng tuần này";
                        }
                    }
                    $room = $this->roomModel->getById($room_id);
                    $room_name = $room['room_name'] ?? 'null';
                    $room_soluong = $room['capacity'] ?? 'null';
                    if (
                        ...
                        $errors['room_id'] = "Phòng $room_name đã có lịch vào thứ $day";
                    }
                    ...
                    if ($this->timetableModel->checkSoLuong($soLuong, $room_id)) {
                        $errors['room_id'] = "Phòng học này có số lượng tối đa :$room_soluong";
                    }
                }
            }
