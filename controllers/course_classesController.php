        ...
        class course_classesController
        {
            ...
            public function editHocPhan()
            {
                ...
                $semester = $this->semesterModel->getActiveSemester();
                if (!$semester) die("Chưa có học kỳ đang hoạt động");
            ...
            public function add()
            {
                ...
                if (isset($_POST['btn_add'])) {
                    ...
                    if (empty($errors)) {
                        ...
                        if ($this->course_classesModel->tonTaiHocPhan(
                            $subject_id,
                            $lecturer_id,
                            $semester_id
                        )) {
                        ...
                        if ($this->timetableModel->phongDaCoLich(
                            $room_id,
                            $day,
                            $session
                        )) {
            ...
            public function edit()
            {
                ...
                if (isset($_POST['btn_edit'])) {
                    ...
                    if (!$lecturer_id) $errors['lecturer_id'] = "Vui lòng chọn giảng viên";
                    if ($max_students <= 0) $errors['max_students'] = "Sĩ số phải > 0";

                    if (!$day) $errors['day_of_week'] = "Chọn thứ học";
                    if (!$session) $errors['session'] = "Chọn buổi học";
                    if (!$room_id) $errors['room_id'] = "Chọn phòng học";

                    if (!$startWeek || !$endWeek || $startWeek > $endWeek) {
                        $errors['week'] = "Tuần học không hợp lệ";
                    }

                    if ($this->timetableModel->phongDaCoLichEdit(
                        $room_id,
                        $day,
                        $session,
                        $id
